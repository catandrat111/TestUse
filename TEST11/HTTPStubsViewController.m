...
#import "HTTPStubsViewController.h"
...
#import "User.h"
@interface HTTPStubsViewController ()
...
@implementation HTTPStubsViewController
...
- (IBAction)downloadText:(UIButton*)sender
{
    ...
    NSString* urlString = @"http://www.opensource.apple.com/source/Git/Git-26/src/git-htmldocs/git-commit.txt?test";
...
//    
   AFHTTPSessionManager * manager = [AFHTTPSessionManager manager];
    ...
    NSString* urlString1 = @"http://www.opensource.apple.com/source/Git/Git-26/src/git-htmldocs/git-commit?test11";
    //json
    [manager GET:urlString1 parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        sender.enabled = YES;
        
        Father* father = [Father objectWithKeyValues:responseObject];
        DDLogInfo(@"%@",[father keyValues]);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
- (IBAction)installTextStub:(UISwitch *)sender
{
    ...
    if (sender.on)
    {
        ...
        textStub = [OHHTTPStubs stubRequestsPassingTest:^BOOL(NSURLRequest *request) {
            // This stub will only configure stub requests for "*.txt" files
            return [request.URL.query containsString:@"test11"];
        } withStubResponse:^OHHTTPStubsResponse *(NSURLRequest *request) {
            // Stub txt files with this
            return [[OHHTTPStubsResponse responseWithFileAtPath:OHPathForFile(@"test12.json", self.class)
                                                     statusCode:200
...
//     }];
    ...
    [manager downloadImageWithURL:[NSURL URLWithString:urlString] options:nil progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
        sender.enabled = YES;
