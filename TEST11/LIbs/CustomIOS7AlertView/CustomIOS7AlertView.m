...
//        self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
        self.frame = CGRectMake(0, 0, self.targetControlller.view.bounds.size.width, self.targetControlller.view.bounds.size.height);
//        delegate = self;
...
//        [[[[UIApplication sharedApplication] windows] firstObject] addSubview:self];
        [self.targetControlller.view addSubview:self];
    }
...
- (UIView *)createContainerView
{
    ...
    screenSize.height = self.targetControlller.view.bounds.size.height;
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if ([[buttonTitles objectAtIndex:i] isEqualToString:@"取消"]) {
            buttonImage = [UIImage imageNamed:@"blueButton"];
        }else{
