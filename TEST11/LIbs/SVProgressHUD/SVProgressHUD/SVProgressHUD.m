...
@property (nonatomic) CFTypeRef retainWindow;
...
- (void)loadingViewStartAnimating
{
    ...
    if ([self.loadingView.layer animationForKey:@"rotationAnimation"]) {
        return;
    }
...
- (void)updatePosition {
	...
	if(string) {
        ...
        self.loadingView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
        self.logoImageView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
        ...
        self.loadingView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
        self.logoImageView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
    ...
    switch (orientation) { 
        ...
        case UIInterfaceOrientationLandscapeLeft:
            ...
            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
                self.window.frame = CGRectMake(0, 0, 768, 1024);
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(!self.overlayView.superview){
        ...
        self.retainWindow = CFBridgingRetain(window);
    }
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
                             ...
                             // NSString *resultUTF8=        [NSString stringWithCString:res encoding:NSUTF8StringEncoding];
...
{
    if (_loadingView == nil) {
        _loadingView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"SVProgressHUD.bundle/读取进度-边框.png"]];
		_loadingView.bounds = CGRectMake(0, 0, 37, 37);
    }
    
    if(!_loadingView.superview)
        [self.hudView addSubview:_loadingView];
...
-(UIImageView *)logoImageView
{
    if (_logoImageView == nil) {
        _logoImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"SVProgressHUD.bundle/读取进度-logo.png"]];
		_logoImageView.bounds = CGRectMake(0, 0, 37, 37);
    }
    
    if(!_logoImageView.superview)
        [self.hudView addSubview:_logoImageView];
...
- (void)performCancelBlock:(ZHBackButtonSelected )block
{
    self.cancelBlock = block;
    
    CGRect hudViewFrame = self.hudView.frame;
    
    if (self.cancelButton == nil) {
        
        UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
        if (orientation == UIInterfaceOrientationLandscapeLeft || orientation ==UIInterfaceOrientationLandscapeRight) {
            self.cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(hudViewFrame.size.height - 24, 4, 20, 20)];
        }else{
            self.cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(hudViewFrame.size.width - 24, 4, 20, 20)];
        }
        
        [self.cancelButton setImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error"] forState:UIControlStateNormal];
        [self.cancelButton addTarget:self action:@selector(backButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        self.cancelButton.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin;
        ...
        UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
        if (orientation == UIInterfaceOrientationLandscapeLeft || orientation ==UIInterfaceOrientationLandscapeRight) {
            self.cancelButton.frame = CGRectMake(hudViewFrame.size.height - 24, 4, 20, 20);
        }else{
            self.cancelButton.frame = CGRectMake(hudViewFrame.size.width - 24, 4, 20, 20);
        }
