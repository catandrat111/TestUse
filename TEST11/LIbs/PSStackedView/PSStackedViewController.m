...
- (CGRect)viewRect {
    ...
    if ((NSFoundationVersionNumber > 1047.25) && (PSIsLandscape())) {
...
- (NSArray *)modifiedRects:(NSArray *)frames newLeft:(CGFloat)newLeft index:(NSUInteger)index {
    ...
    CGRect prevFrame;
    for (int i = (int)index; i < [modifiedFrames count]; i++) {
...
- (void)updateViewControllerMasksAndShadow {   
    if (enableShadows_ == YES) {
        ...
        int from = (int)self.viewControllers.count-1;
        int cleft = 10000;
...
- (void)pushViewController:(UIViewController *)viewController fromViewController:(UIViewController *)baseViewController animated:(BOOL)animated statusBar:(BOOL)isStatusBar; {
    // figure out where to push, and if we need to get rid of some viewControllers
    if (baseViewController) {
        [self.viewControllers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            UIViewController *baseVC = objc_getAssociatedObject(obj, kPSSVAssociatedBaseViewControllerKey);
...
- (BOOL)shouldSnapAnimate {
    BOOL shouldSnapAnimate = abs((int)lastDragOffset_) > 10;
