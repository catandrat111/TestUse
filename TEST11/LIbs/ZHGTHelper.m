...
//            //首先获取id
//            [[ZHNetworkEngine sharedInstance] getTestIdOnFinished:^(id response) {
//                NSString* gt_id = (NSString*)response;
//                if (gt_id.length > 0 ) {
//                    //开启验证
//                    BOOL isOpenGT = [manager requestGTest:gt_id withChallenge:GT_chanllenge];
//                    if (isOpenGT) {//成功
//                         [self openGT];
//                    }
//                    else {
...
- (void)setUpGETestWithGtId:(NSString *)gt_id GT_chanllenge:(NSString *)GT_chanllenge
{
    if (gt_id.length > 0 ) {
        //开启验证
        BOOL isOpenGT = [[GTManager sharedGTManger] requestGTest:gt_id withChallenge:GT_chanllenge];
        if (isOpenGT) {//成功
            [self openGT];
        }
        else {
